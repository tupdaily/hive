...
export class AgentManager {
  ...
  async createAgent(userId: string, agentData: {
    ...
    // Always fetch all MCP tools from the server and attach to every agent.
    // Replace "Composio" with your MCP server name if needed.
    let mcpTools: any[] = [];
    try {
      ...
      console.error('Unable to list MCP tools for server Composio. No tools will be attached:', err instanceof Error ? err.message : String(err));
      mcpTools = [];
    }
...
Improvements needed: 
    ...
    try {
      ...
      console.log('Attaching MCP tool ids to new agent:', toolIds);
      ...
      const createPayload: any = {
        ...
        toolIds // Always include, even if empty
      };
